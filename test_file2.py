...
# Mock Tkinter widgets as a fixture
@pytest.fixture
def mock_tkinter_widgets():
    # Create mock Tkinter widget objects
    ...
    # Return a dictionary of mock widgets for easy access
    return {
...
# Function to test the scenario where an error occurs while loading the file
def test_select_input_file_error(mock_tkinter_widgets):
    # Assign the global variables as used in the select_input_file function
    ...
    # Initialize global variables using the mock objects
    ...
    # Mock filedialog.askopenfilename to return a valid file path
    with mock.patch.object(filedialog, 'askopenfilename', return_value="test_file.xlsx"):
        # Mock pd.read_excel to raise an exception (simulate a loading error)
        with mock.patch.object(pd, 'read_excel', side_effect=Exception("Failed to read file")):
            # Call the function and ensure an error message is shown
...
# Function to test the success case where the file is loaded successfully
def test_select_input_file_success(mock_tkinter_widgets):
    # Assign the global variables as used in the select_input_file function
    global input_file_path, coordinate_column, specimen_name_column, description_column
    global coord_dropdown, name_dropdown, desc_dropdown
    ...
    # Mock filedialog.askopenfilename to return a specific file path
    with mock.patch.object(filedialog, 'askopenfilename', return_value="test_file.xlsx"):
        # Mock pd.read_excel to return a predefined DataFrame
        mock_df = pd.DataFrame({
            'Coordinate': ['1,1'],
            'Specimen Name': ['Specimen A'],
            'Description': ['A rare specimen'],
        })
        with mock.patch.object(pd, 'read_excel', return_value=mock_df):
            # Call the function
            select_input_file()

            # Check if the correct file path is set
            mock_tkinter_widgets['input_file_path'].set.assert_called_with("test_file.xlsx")

            # Check if the columns from the mock DataFrame are added to the dropdowns
            mock_coord_dropdown.menu.add_command.assert_any_call(label='Coordinate', command=mock.ANY)
            mock_name_dropdown.menu.add_command.assert_any_call(label='Specimen Name', command=mock.ANY)
            mock_desc_dropdown.menu.add_command.assert_any_call(label='Description', command=mock.ANY)
